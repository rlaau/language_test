...
func (lx *Lexer) Next() token.Token {
	...
	// 으로 "분할"된다. (서로소가 아닌 경우엔, 우선순위에 의해 결정적으로 분할됨.)

	// 우선 EOF의 경우를 제거하자
	// ILLEGAL은 토크나이징 실패 케이스로써 처리하자
	if lx.isOvered() {
		return token.NewToken(token.EOF, lx.currentPosition)
	}
	...
	if isAlpha(lx.currentByte()) {
		...
		keyWord, isKeyWordExist := token.IsKeyWord(string(candidate))
		// 키워드인 경우 <-> 1번 케이스
		if isKeyWordExist {
			return token.NewToken(keyWord, lx.currentPosition)
		}
		// 식별자인 경우 <-> 2번 케이스
		idToken := token.NewToken(token.ID, lx.currentPosition)
	...
	isOmitStart := func(s string) (token.TokenKind, bool) {
		if s == "(" {
			return token.OMIT, true
		}
		return token.ILLLEGAL, false
	}
	isOmitEnd := func(s string) (token.TokenKind, bool) {
		if s == ")" {
			return token.OMIT, true
		}
		return token.ILLLEGAL, false
	...
	if isDigit(lx.currentByte()) {
		...
		numberToken := token.NewToken(token.NUMBER, lx.currentPosition)
	...
	isStrEdge := func(s string) (token.TokenKind, bool) {
		if s == "\"" {
			return token.STRLIT, true
		}
		return token.ILLLEGAL, false
	}
	...
	if lx.isNextExist() {
		...
		if tokenKind, isOperator := lx.readIf(candidate, token.IsOperator); isOperator {
			return token.NewToken(tokenKind, lx.currentPosition)
		}
		// 한 칸 이동 후 토크나이징 시도했지만 실패 시, 다시 원위치로 롤백
		rollBack()
	}
	if tokenKind, isOperator := lx.readIf(string(lx.currentByte()), token.IsOperator); isOperator {
		return token.NewToken(tokenKind, lx.currentPosition)
	}

	// 이제 7케이스만이 남는다.
	// 7의 구분자 집합에 대해, 모든 원소의 길이가 1임을 상정한다.
	if tokenKind, isDelimeter := lx.readIf(string(lx.currentByte()), token.IsDelimeter); isDelimeter {
		return token.NewToken(tokenKind, lx.currentPosition)
	}
...
// 상태 변경 함수다.
func (lx *Lexer) readIf(candidate string, isToken func(string) (token.TokenKind, bool)) (token.TokenKind, bool) {
